	...
	<?php foreach($site->children()->children()->visible() as $exhibition): ?>
		...
		<?php if($exhibition->currentupcoming() == 'current'): ?>
			...
			<div class="section group exhibitions">
				<div class="col span_5_of_10" style="margin: 0%">
					<h1>
						<a href="<?= $exhibition->url() ?>">
							<?php 
							if (strlen($exhibition->author())) {
								echo $exhibition->author();
							}else{
								echo $exhibition->artist(); 
							} 
							?>
						</a>
					</h1>
					...
					<i class="exhibition-title">
						<a href="<?= $exhibition->url() ?>">
							...
							if (strlen($exhibition->booktitle())) {
								echo $exhibition->booktitle();
							}
							elseif (strlen($exhibition->exhibition())){
								echo $exhibition->exhibition();
							}
							else {
								echo $exhibition->programmetitle(); 
							} 
					...
					<h2>
						...
						<?php if(strlen($exhibition->startdate()) && strlen($exhibition->enddate())): ?>

							<?php $startDate = new DateTime( $exhibition->startdate() ); ?>
							<?php $endDate = new DateTime( $exhibition->enddate() ); ?>  

							<?php if ( $startDate->format('F') == $endDate->format('F')): ?>      
								<span><?= $startDate->format('d'); ?></span>&mdash;<span><?= $endDate->format('d F Y'); ?></span>
							<?php else: ?>              
								<span><?= $startDate->format('d F'); ?></span>&mdash;<span><?= $endDate->format('d F Y'); ?></span>           
							<?php endif ?>
